...
const Login = () => {
  const navigate=useNavigate();
    ...
    const isLoggedIn=useSelector((state:any)=>{//getting the status of user login from redux
      return state.login.auth;
    })
    useEffect(()=>{//redirecting to homepage if the user is alread logged in
    if(isLoggedIn){
       navigate('/adminpanel');
    }
    },[isLoggedIn])
              ...
              <CCard className="p-4">
                <CCardBody>
                  <CForm onSubmit={formSubmitHandler}>  
                    ...
                    {isLoggedIn?'alread logged in':'Sign in to your account'}
                    </p>
                    ...
                    <CInputGroup className="mb-4">
                      ...
                      <CInputGroupText className="border-start-0">
                        {!showPassword?<AiFillEyeInvisible onClick={passwordToggler} />:<AiFillEye onClick={passwordToggler}/>}
                       
                      </CInputGroupText>
                    </CInputGroup>
                    <CRow>
                      <CCol xs={6}>
                        <CButton type="submit" className="px-4 text-light btn-picco">
                          Login
                        </CButton>
                      </CCol>
